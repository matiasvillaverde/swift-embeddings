    ...
    public static func loadModel(
        ...
        let embeddings = try Clip.Embeddings(
            tokenEmbedding: MLTensorUtils.embedding(
                weight: safetensors.mlTensor(
                    forKey: weightKeyTransform("text_model.embeddings.token_embedding.weight"))),
            positionEmbeddingWeight: safetensors.mlTensor(
                forKey: weightKeyTransform("text_model.embeddings.position_embedding.weight")))
        ...
        for i in 0..<config.textConfig.numHiddenLayers {
            ...
            let mlp = try Clip.MLP(
                fc1: MLTensorUtils.linear(
                    weight: safetensors.mlTensor(
                        forKey: weightKeyTransform("text_model.encoder.layers.\(i).mlp.fc1.weight")),
                    bias: safetensors.mlTensor(
                        forKey: weightKeyTransform("text_model.encoder.layers.\(i).mlp.fc1.bias"))),
                fc2: MLTensorUtils.linear(
                    weight: safetensors.mlTensor(
                        forKey: weightKeyTransform("text_model.encoder.layers.\(i).mlp.fc2.weight")),
                    bias: safetensors.mlTensor(
                        forKey: weightKeyTransform("text_model.encoder.layers.\(i).mlp.fc2.bias"))))
        ...
        return try Clip.TextModel(
            ...
            finalLayerNorm: MLTensorUtils.layerNorm(
                weight: safetensors.mlTensor(
                    forKey: weightKeyTransform("text_model.final_layer_norm.weight")),
                bias: safetensors.mlTensor(
                    forKey: weightKeyTransform("text_model.final_layer_norm.bias")),
                epsilon: config.textConfig.layerNormEps),
            textProjection: MLTensorUtils.linear(
                weight: safetensors.mlTensor(
                    forKey: weightKeyTransform("text_projection.weight"))))
    }
}
